...
//------------------- main ----------------------------
#define FIELD_VALIDATORS 0
#define ADDING_FORM_VALIDATORS 0
#define TODO_1 0
#define TODO_2 0
...
int main()
{
	...
	auto yearField = std::make_unique<Field<int>>("Which year is it in your studies? (1-" + std::to_string(MAX_POSSIBLE_YEAR) + ")");
	auto courseField = std::make_unique<Field<short>>("How many courses you are taking this year?");
	auto facultyField = std::make_unique<Field<FacultyValue>>
		("What faculty are you registering to?\n(1-Computer Science, 2-Medicine, 3-Literature)");
	...
	facultyField->addValidator(facultyValidator.get()); // welcome back!!

	// Creating form validators
	auto facultyToYearValidator
		= std::make_unique<Faculty2YearValidator<Field<FacultyValue>, Field<int>>>(
			facultyField.get(),
			yearField.get());
	auto coursesToYearValidator
		= std::make_unique<Courses2YearValidator<Field<short>, Field<int>>>(
			courseField.get(),
			yearField.get());
#if FIELD_VALIDATORS

#endif // CREATING_THE_FIELD_VALIDATORS
...
#if ADDING_FORM_VALIDATORS

	// Adding form validators
#endif // ADDING FORM VALIDATORS
